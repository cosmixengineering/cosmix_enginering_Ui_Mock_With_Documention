...
- Tender document workspace (2026-09-18): Sales requires reusable tender soft-copy templates where repeated content stays controlled and only designated tender fields change. The Sales mock includes template selection, editable tender particulars, locked/linked section visibility, ordered PDF attachments, light/balanced/maximum compression profiles, original-file preservation, print/Save-as-PDF for the generated tender portion and package history. Final binary merge/compression of 250–300 MB PDFs requires a production server/background document worker with progress, recovery and secure temporary storage; the HTML mock records this complete job specification without claiming that large binaries were processed.
