        ...
        <div class="flex-1 flex flex-col h-screen overflow-hidden bg-[#f8fafc]">
            ...
            <div class="bg-white text-gray-700 text-xs font-semibold py-3 px-6 flex justify-between items-center z-40 border-t border-gray-200 shadow-[0_-2px_10px_rgba(0,0,0,0.05)]">
                <div class="flex gap-5">
                    <button class="hover:text-blue-600 transition flex items-center gap-1.5" onclick="openModal('Quick Calculator', '<div class=\'flex flex-col gap-3\'><label class=\'text-xs font-semibold text-gray-600\'>Enter expression (e.g. 5000 * 0.18)</label><input type=\'text\' class=\'w-full border border-gray-300 rounded p-2 text-sm\' oninput=\'try{document.getElementById(&quot;calc-result&quot;).innerText=eval(this.value.replace(/[^0-9+\\-\\*\\/\\(\\).]/g, &quot;&quot;))||0}catch(e){document.getElementById(&quot;calc-result&quot;).innerText=&quot;...&quot;}\'><div class=\'bg-gray-100 p-3 rounded font-mono text-xl font-bold text-right text-gray-800\' id=\'calc-result\'>0</div></div>', null)"><i class="fas fa-calculator text-blue-500"></i> Calculator</button>
                      <button class="hover:text-amber-600 transition flex items-center gap-1.5" onclick="openModal('My Notes', '<textarea id=\'erp-notes-box\' class=\'w-full border border-gray-300 rounded p-2 text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500 outline-none\' rows=6 placeholder=\'Type your quick notes here...\'></textarea>', ()=>{localStorage.setItem('erp_notes',document.getElementById('erp-notes-box').value);showToast('Notes saved securely', 'success');}); setTimeout(()=>document.getElementById('erp-notes-box').value=localStorage.getItem('erp_notes')||'', 50)"><i class="fas fa-sticky-note text-amber-500"></i> My Notes</button>
                      <button class="hover:text-red-600 transition flex items-center gap-1.5" onclick="if(confirm('Are you sure you want to reset all mock data (sales, vendor, HR)? This cannot be undone.')){localStorage.clear();location.reload();}"><i class="fas fa-trash-alt text-red-500"></i> Reset App Data</button>
                </div>
